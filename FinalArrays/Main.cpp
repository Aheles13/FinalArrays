#include <iostream>
#include <cstdlib>
#include <ctime>
#include <algorithm>

int main() {
	setlocale(LC_ALL, "Russian");
	int n, m;
	int sum;


	//std::cout << "Hello world!\n\n";


	// Задача 1. Сортировка половины массива.

	/*std::cout << "Задача 1.\nИзначальный массив\n";
	const int size1 = 10;
	int arr1[size1];
	srand(time(NULL));
	for(int i = 0; i < size1; i++){
		arr1[i] = rand() % (10 + 1 - 1) + 1;  //[1...10] (10 + 1)
			std::cout << arr1[i] << " ";
	}
	std::cout << std::endl;


	// Пузырькова сортировка
	//for (int i = 4; i > 0; i--)
		//for (int j = 0; j < i; j++)
			//if (arr1[j] > arr1[j + 1])
				//std::swap(arr1[j], arr1[j + 1]);
	// Быстрая сортировка
	std::sort(arr1,arr1 + 5);


	std::cout << "Итоговый массив\n";
	for (int i = 0; i < size1; i++)
		std::cout << arr1[i] << ' ';
	std::cout << "\n\n";*/


	//Задача 2. Темпратура в январе 
	/*std::cout << "Задача 2.\nТемпература за все дни января:\n";
	const int size2 = 31;
	int arr2[size2];
	srand(time(NULL));
	for (int i = 0; i < size2; i++) {
		arr2[i] = rand() % (-1 + 1 - (-30)) + (-30);  // rand % 30 - 30 [-30..-1]
		std::cout << arr2[i] << ", ";
	}
	std::cout << "\b\b.\n ";
	sum = 0;                //Сумма для средне арефметического 
	int counter = 0;          // Кол дней для < n
	std::cout << "Введите максимальную границу температуры (от - 30 до -1 градусов) ";
	std::cin >> n;
	for (int i = 0; i < size2; i++) {
		sum += arr2[i];
	if (arr2[i] < n)
		counter++;
}
std::cout << "Количество дней когда температура опускалась ниже" << n << "градусов: " << counter << '\n';
std::cout << "Средняя температура за месяц: " << (double)sum / size2 << "\n\n";*/


	//Задача 3. Вхождение в диапазон 
	/*std::cout << "Задача3.\nВведите начало диапазона ->";
	std::cin >> n;
	std::cout << "Введите конец диапазона ->";
	std::cin >> m;
	const int size3 = 20;
	int arr3[size3];
	srand(time(NULL));
	std::cout << "Вывод:\n";
	for (int i = 0; i < size3; i++) {
		arr3[i] = rand() % 100 + 1;  // [0..100]
		if (arr3[i] > n && arr3[i] < m)
			std::cout << i + 1 << ". " << arr3[i] << "!\n";
		else
			std::cout << i + 1 << ". " << arr3[i] << "\n";
	}
	std::cout << std::endl;*/
	 
	//Задача 4.
	std::cout << "Задача 1.\nИзначальный массив\n";
	const int size1 = 10;
	int arr1[size1];
	srand(time(NULL));
	for (int i = 0; i < size1; i++) {
		arr1[i] = rand() % (10 + 1 - 1) + 1;  //[1...10] (10 + 1)
		std::cout << arr1[i] << " ";
	}
	std::cout << std::endl;

	std::cout << "Числа больше преведушего.\n";

	for (int i = 0; i < size1; i++) {
		if (arr1[i] > size1)
			std::cout << size1;


	}

	
	



	return 0;
}